pipeline {
    agent any
    environment {
        DOCKER_HUB_REPO = "zey123/studybuddy"
    ...
    stages {
        ...
        stage('Update Deployment YAML with New Tag') {
            steps {
                script {
                    sh """
                    sed -i 's|image: zey123/studybuddy:.*|image: zey123/studybuddy:${IMAGE_TAG}|' manifests/deployment.yaml
                    """
                }
            }
        }
            ...
            steps {
                script {
                    kubeconfig(credentialsId: 'kubeconfig', serverUrl: 'https://192.168.49.2:8443') {
                        sh '''
                        argocd login 34.132.117.45:31704 --username admin --password $(kubectl get secret -n argocd argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d) --insecure
